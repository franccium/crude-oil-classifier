...
def select_featureset():
    ...
    tk.Button(root, text="Density Group + CII", width=25,
              command=lambda: set_choice(featureset_density_group_CII)).pack(pady=5)
    root.mainloop()
...
def select_graphs():
    ...
    tk.Checkbutton(root, text="Show feature space visualizations", variable=flags['feature_space']).pack(anchor='w',
                                                                                                         padx=20)
    tk.Checkbutton(root, text="Show data scatter plots", variable=flags['data_scatter']).pack(anchor='w', padx=20)
    tk.Checkbutton(root, text="Show cross-validation summary graphs", variable=flags['cv_summary']).pack(anchor='w',
                                                                                                         padx=20)
